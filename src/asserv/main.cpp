...
int main() {
	...
	// Init Encoders
	Encoder *lEnc = new Encoder(LEFT_INCREMENTAL_A_PIN, LEFT_INCREMENTAL_B_PIN, ENCODER_LEFT_REVERSE, 0, pio0/*, 3952*/);
	...
	// Setup PID Telem names
	lSpeedPid->telem.setName("Left Speed PID");
	rSpeedPid->telem.setName("Right Speed PID");
	dstPid->telem.setName("Dst PID");
	anglePid->telem.setName("Angle PID");
	...
	INA236 *curr = nullptr;
#ifdef PAMINI
	curr = new INA236(ACCESSORY_I2C_INST, ACCESSORY_I2C_SDA, ACCESSORY_I2C_SCL, INA236_ADDR, INA236_SHUNT_RESISTOR, INA236_MAX_CURRENT, ACCESSORY_I2C_BAUDRATE);
	//uint16_t mid = curr->readManufacturerID();
	//uint16_t did = curr->readDeviceID();
	//printf("%x %x\n", mid, did);
	curr->setAveraging(INA236Avg::a4);
#endif
	...
	ControlLoop *cl = new ControlLoop(lEnc, rEnc, lDrv, rDrv, odo,
									...
									ctrl, ENCODER_WHEEL_RADIUS, POSITION_DOWNSAMPLING, curr);
...
#ifdef PAMINI // Pamini
	// Piezo setup
	piezo = new Piezo(PIEZO_PIN, 108.7e3);
	...
	strip2->setLedParams(3, LedFunction::brakeLight, LedPosition::center | LedPosition::rear);
	...
	strip2->setLedParamsRange(4, 4+2, LedFunction::ringLight | LedFunction::fancyBlinker, LedPosition::rear | LedPosition::right);
	strip2->setLedParamsRange(WS2812B2_COUNT-3-2, WS2812B2_COUNT-3, LedFunction::ringLight | LedFunction::fancyBlinker, LedPosition::rear | LedPosition::left);

	strip2->setLedOrderRange(0, 3, false); // RGB on WS2811
	strip2->setLedOrderRange(4, WS2812B2_COUNT-3, true); // GRB on WS2812B (default, but still put) 
	strip2->setLedOrderRange(WS2812B2_COUNT-2, WS2812B2_COUNT-1, false); // RGB on WS2811
	...
	cachedLeds->cacheRange(LedFunction::brakeLight, LedPosition::rear | LedPosition::center, true);
	cachedLeds->cacheRange(LedFunction::headlight);
	cachedLeds->cacheRange(LedFunction::all ^ LedFunction::ringLight, LedPosition::left);
